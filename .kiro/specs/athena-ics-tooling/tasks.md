...
- [ ] 1. Add shared ICS types to athena-common
  - [ ] 1.1 Add ICS type definitions to `crates/athena-common/src/lib.rs`
...
- [ ] 2. Implement Modbus TCP frame encoding/decoding
  - [ ] 2.1 Create `crates/athena-modbus/` crate with Cargo.toml
  ...
  - [ ] 2.2 Implement MBAP frame encoding/decoding in `src/frame.rs`
...
- [ ] 3. Implement Modbus TCP client and read operations
  - [ ] 3.1 Implement async Modbus TCP client in `src/client.rs`
  ...
  - [ ] 3.2 Implement Modbus read operations in `src/lib.rs`
...
- [ ] 4. Implement Modbus write operations and safety validation
  - [ ] 4.1 Implement safe-range validation in `src/lib.rs`
  ...
  - [ ] 4.3 Implement Modbus write operations in `src/lib.rs`
...
- [ ] 5. Implement Modbus enumeration and fuzzing
  - [ ] 5.1 Implement Unit ID enumeration in `src/lib.rs`
  ...
  - [ ] 5.2 Implement Modbus fuzzer in `src/fuzz.rs`
...
- [ ] 6. Implement Modbus CLI entry point
  - [ ] 6.1 Implement `src/main.rs` for athena-modbus
...
- [ ] 7. Checkpoint — Modbus module complete
  - Ensure all Modbus tests pass (`cargo test -p athena-modbus`)
  - Ensure `cargo build -p athena-modbus` produces binary
  - Ask the user if questions arise.

- [ ] 8. Implement CAN frame crafting and validation
  - [ ] 8.1 Create `crates/athena-canbus/` crate with Cargo.toml
    - Add workspace member in root `Cargo.toml`
    - Dependencies: `athena-common`, `tokio`, `clap`, `serde`, `serde_json`, `rand`, `rand_xoshiro`, `libc`
    - Dev-dependencies: `proptest`, `tokio-test`
    - _Requirements: 12.7_

  - [ ] 8.2 Implement CAN frame validation and crafting in `src/lib.rs`
...
- [ ] 9. Implement CAN SocketCAN operations
  - [ ] 9.1 Implement SocketCAN raw socket wrapper in `src/socket.rs`
  ...
  - [ ] 9.2 Implement CAN sniff in `src/sniff.rs`
  ...
  - [ ] 9.3 Implement CAN replay in `src/replay.rs`
  ...
  - [ ] 9.4 Implement CAN injection in `src/lib.rs`
...
- [ ] 10. Implement CAN fuzzer
  - [ ] 10.1 Implement CAN fuzzer in `src/fuzz.rs`
    - Use `Xoshiro256PlusPlus` PRNG seeded from config
...
- [ ] 11. Implement CAN CLI entry point
  - [ ] 11.1 Implement `src/main.rs` for athena-canbus
...
- [ ] 12. Checkpoint — CAN module complete
  - Ensure all CAN tests pass (`cargo test -p athena-canbus`)
  - Ensure `cargo build -p athena-canbus` produces binary
  - Ask the user if questions arise.

- [ ] 13. Implement ICS safety controls in the orchestrator
  - [ ] 13.1 Create `orchestrator/ics_safety.py`
  ...
  - [ ] 13.3 Integrate ICS safety into orchestrator OPAR loop
...
- [ ] 14. Update tool registry with ICS entries
  - [ ] 14.1 Add Modbus and CAN tool entries to `config/tool-registry.toml`
...
- [ ] 15. Add ICS target configuration files
  - [ ] 15.1 Create target configuration files
...
- [ ] 16. Implement ICS eval metrics
  - [ ] 16.1 Create `eval/ics_metrics.py`
