...
nohup kubectl port-forward -n nexus-soc svc/wazuh-dashboard 5601:5601 > /dev/null 2>&1 &
echo "Wazuh Dashboard exposed at https://localhost:5601"

# MinIO Console (HTTP)
nohup kubectl port-forward -n soc svc/minio 9001:9001 > /dev/null 2>&1 &
echo "MinIO Console exposed at http://localhost:9001"

# Jupyter Workbench (HTTP)
nohup kubectl port-forward -n soc svc/nexus-workbench 8888:8888 > /dev/null 2>&1 &
