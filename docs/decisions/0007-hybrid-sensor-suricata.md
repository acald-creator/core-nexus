...
- Runtime / kernel telemetry (e.g. Tetragon) **complements** Suricata; it does not
  replace it.
- Wazuh is the near-term SOC event store; Suricata events feed Wazuh / Vector /
  enrichment as routed.
...
- Athena/OPAR ground-truth evaluation assumes Suricata (and Wazuh) can observe
  labeled traffic.
