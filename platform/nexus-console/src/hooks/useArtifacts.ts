...
export function useArtifacts(category: ArtifactCategory) {
  ...
  const { data: objects = [], isLoading, error } = useQuery<ArtifactObject[]>({
    ...
    queryFn: async () => {
      const response = await fetchWithAuth(`${config.apiGatewayUrl}/api/v1/artifacts?category=${category}`);
  ...
  const getDownloadUrl = async (key: string): Promise<string> => {
    const response = await fetchWithAuth(`${config.apiGatewayUrl}/api/v1/artifacts/${encodeURIComponent(key)}/download-url`);
